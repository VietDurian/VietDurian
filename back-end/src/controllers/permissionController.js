...
const getPermissionRequestDetail = async (req, res, next) => {
  try {
    ...
    const request =
      await permissionService.getPermissionRequestDetail(request_id);
    if (!request) {
      return res
        .status(404)
...
const getVerifyCCCDStatus = async (req, res, next) => {
  try {
    const userId = req.user._id;
    const result = await permissionService.getVerifyCCCDStatus(userId);
    return res.status(200).json({
      code: 200,
      message: "Get verify_cccd status success",
      data: result,
    });
  } catch (err) {
    return next(err);
