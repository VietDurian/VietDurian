...
const MOCK_PRODUCT = {
  ...
  description:
    "Sầu riêng Ri6 là giống sầu riêng phổ biến nhất tại Tiền Giang, nổi tiếng với múi dày, màu vàng ươm, hạt lép nhỏ và hương thơm đặc trưng nồng nàn.",
  price: 180000,
  origin: "Tiền Giang",
  weight: 3.5,
  view_count: 1240,
...
export default function ProductDetailScreen() {
              ...
              >
                {product.images.map((img, idx) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.thumbWrap,
                      selectedImage === idx && styles.thumbWrapActive,
                    ]}
                  >
                    <Image
                      source={{ uri: img.url }}
  ...
  <Text style={{ fontSize: 16, fontWeight: "600", color: "#059669" }}>/ kg</Text>
            ...
            <View style={styles.metaList}>
              <MetaRow
                icon={<Feather name="map-pin" size={17} color="#059669" />}
                label="Xuất xứ:"
              ...
              {product.type_id && (
                <MetaRow
                  icon={<Feather name="tag" size={17} color="#059669" />}
                  label="Loại sản phẩm:"
                  value={product.type_id.name}
                />
              )}
              {(product.harvest_start_date || product.harvest_end_date) && (
                <MetaRow
                  icon={<Feather name="calendar" size={17} color="#059669" />}
                  label="Mùa vụ:"
              ...
              {activeTab === "specifications" && (
                <View>
                  {[
                    ...
                    {
                      label: "Mùa vụ bắt đầu",
                      value: formatDate(product.harvest_start_date),
                    },
                    {
                      label: "Trạng thái",
                      value:
                    ...
                    {
                      label: "Mùa vụ kết thúc",
                      value: formatDate(product.harvest_end_date),
                    },
                  ].map((row, i) => (
                    <View
                      key={i}
...
function MetaRow({ icon, label, value, green }) {
  return (
    <View style={styles.metaRow}>
      ...
      <Text style={[styles.metaValue, green && { color: "#16A34A" }]}>
        {value}
      </Text>
    </View>
  );
