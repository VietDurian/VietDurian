...
export function TypePage() {
  ...
  const fetchProductTypes = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error.message ||
          t("fetch_type_failed") ||
          "Lấy danh sách loại sầu riêng thất bại",
      );
    } finally {
      setLoading(false);
    }
  ...
  const formatDate = (value) => {
    ...
    if (Number.isNaN(d.getTime()))
      return t("invalid_date") || "Ngày không hợp lệ";
  ...
  const checkDuplicateName = async (name, excludeId = null) => {
    ...
    return productTypes.some((type) => {
      const shouldExclude =
        excludeId && (type._id === excludeId || type.id === excludeId);
      if (shouldExclude) return false;

      const normalizedExisting = (type.name || "")
        .replace(/\s+/g, "")
        .toLowerCase();
  ...
  const handleNameBlur = async () => {
    if (!formData.name.trim()) {
      setNameError(
        t("type_name_required") || "Tên loại sầu riêng không được để trống",
      );
      return;
    }
  ...
  const handleCreate = async () => {
    ...
    if (!formData.description.trim()) {
      toast.error(
        t("type_description_required") || "Mô tả không được để trống",
      );
      return;
    }
    ...
    try {
      ...
      toast.success(
        t("create_type_success") || "Tạo loại sầu riêng thành công",
      );
      closeModals();
      ...
      // Xử lý lỗi trùng tên
      if (
        error.message === "Type product name already exists" ||
        error.response?.status === 400
      ) {
        toast.error(
          t("type_name_duplicate") || "Tên loại sầu riêng đã tồn tại",
        );
      } else {
        toast.error(
          error.message ||
            t("create_type_failed") ||
            "Tạo loại sầu riêng thất bại",
        );
      }
    } finally {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      await productTypesAPI.updateProductType(
        selectedType._id || selectedType.id,
        updateData,
      );
      toast.success(
        t("update_type_success") || "Cập nhật loại sầu riêng thành công",
      );
      closeModals();
      // Fetch lại data để đảm bảo hiển thị đúng
      await fetchProductTypes();
      ...
      if (
        ...
        toast.error(
          error.message ||
            t("update_type_failed") ||
            "Cập nhật loại sầu riêng thất bại",
        );
      }
    } finally {
      setActionLoading(null);
  ...
  const handleDelete = async () => {
    ...
    try {
      await productTypesAPI.deleteProductType(
        selectedType._id || selectedType.id,
      );
      toast.success(
        t("delete_type_success") || "Xóa loại sầu riêng thành công",
      );
      closeModals();
      // Fetch lại data để đảm bảo hiển thị đúng
      await fetchProductTypes();
      ...
      toast.error(
        error.message ||
          t("delete_type_failed") ||
          "Xóa loại sầu riêng thất bại",
      );
    } finally {
      setActionLoading(null);
    }
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder={
                t("search_product_types") ||
                "Tìm theo tên loại sầu riêng, mô tả..."
              }
              value={searchTerm}
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-linear-to-br from-[#1a4d2e] to-[#2d7a4f] flex items-center justify-center text-white font-bold">
                    ...
                    <td className="px-6 py-4">
                      <p className="text-sm text-gray-900 whitespace-pre-line wrap-break-word max-w-md">
                        {type.description ||
                          t("no_description") ||
                          "Chưa có mô tả"}
                      </p>
          ...
          filteredTypes.map((type) => (
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                <div className="w-12 h-12 rounded-full bg-linear-to-br from-[#1a4d2e] to-[#2d7a4f] flex items-center justify-center text-white font-bold shrink-0">
            ...
            <div className="space-y-5">
              <div>
                ...
                <input
                  ...
                  placeholder={
                    t("enter_product_type_name") || "Nhập tên loại sầu riêng"
                  }
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={10}
                  className="w-full min-h-65 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1a4d2e] bg-white text-gray-900 whitespace-pre-wrap"
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-sm text-gray-600">
                  {t("confirm_delete_product_type") ||
                    "Bạn có chắc muốn xóa loại sầu riêng này?"}
                </p>
