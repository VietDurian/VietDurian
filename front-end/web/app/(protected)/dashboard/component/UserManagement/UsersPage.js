...
export function UsersPage() {
	...
	const fetchUsers = useCallback(
		async (params = {}) => {
			...
			try {
				...
				const pgn = res?.pagination || res?.data?.pagination || null;
				...
				setPagination(
					pgn
						? {
							...
							currentPage: Number(pgn.currentPage ?? requestedPage),
	...
	useEffect(() => {
		...
		fetchUsers(params);
	}, [page, searchTerm, roleFilter, statusFilter, user?._id, fetchUsers]);
			...
			<div className="bg-white rounded-xl p-4 md:p-6 shadow-sm border border-gray-100 mb-6">
				<div className="flex flex-col md:flex-row gap-4">
					...
					>
						...
						<option value="contentExpert">
							{t("contentExpert") || "Content Expert"}
						</option>
					</select>
