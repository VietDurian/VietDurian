	...
	return (
		<div className="w-14 h-14 md:w-16 md:h-16 rounded-xl overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center shrink-0">
			{src ? (
				...
				<img src={src} alt={alt || 'product'} className="w-full h-full object-cover" />
			) : (
...
export function ProductRatingWidget({ limit = 50 } = {}) {
	...
	useEffect(() => {
		...
		const run = async () => {
			...
			try {
				const res = await productsAdminAPI.getAllProducts({ page: 1, limit: Number(limit) || 50 });
	...
	const sorted = useMemo(() => {
		return [...(Array.isArray(products) ? products : [])].sort((a, b) => Number(b?.rating || 0) - Number(a?.rating || 0));
	}, [products]);
