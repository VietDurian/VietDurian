      ...
      {!image ? (
        ...
        >
          ...
          <p className="text-sm font-medium text-gray-600">
            {t("create_blog_image_click")}
          </p>
          <p className="text-xs text-gray-400 mt-1">
            {t("create_blog_image_hint")}
          </p>
        </div>
      ) : (
...
export default function CreateBlogPage() {
  ...
  const handleBlogImageChange = (event) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setError(t("create_blog_image_too_large"));
      return;
    }
  ...
  const handleProceedToStep2 = () => {
    if (!canProceedToStep2()) {
      setError(t("create_blog_step1_required"));
      return;
    }
    setError("");
    setCurrentStep(2);
  ...
  const handleSubmitBlog = async () => {
    if (knowledgeBlocks.length === 0) {
      setError(t("create_blog_chapter_required"));
      return;
    }
          ...
          {currentStep === 1 && (
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6 flex items-center gap-2">
                ...
                {t("create_blog_info_title")}
              </h2>
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    {t("create_blog_title_label")}{" "}
                  ...
                  <input
                    ...
                    placeholder={t("create_blog_title_placeholder")}
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    {t("create_blog_intro_label")}{" "}
                    <span className="text-red-500">*</span>
                  </label>
                  <textarea
                    value={blogContent}
                    onChange={(e) => setBlogContent(e.target.value)}
                    placeholder={t("create_blog_intro_placeholder")}
              ...
              <div className="bg-white rounded-2xl shadow-lg p-8">
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                    <BookOpen className="text-emerald-500" size={28} />
                    {t("create_blog_chapters_title")} ({knowledgeBlocks.length})
                  </h2>
                  {!isAddingBlock && (
                    <button
                ...
                {knowledgeBlocks.length === 0 && !isAddingBlock && (
                  <div className="text-center py-12">
                    ...
                    >
                      <Plus size={20} />
                      {t("create_blog_add_first_chapter")}
                    </button>
                  </div>
                )}
              </div>
              ...
              <Modal
                ...
                title={
                  editingBlockIndex !== null
                    ? t("create_blog_block_modal_edit")
                    : t("create_blog_block_modal_add")
                }
              >
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      {t("create_blog_block_title_label")}{" "}
                      <span className="text-red-500">*</span>
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">
                      {t("create_blog_block_content_label")}{" "}
                      <span className="text-red-500">*</span>
                    </label>
                    <textarea
                      value={blockContent}
                  ...
                  <ImageUpload
                    ...
                    label={
                      <>
                        {t("create_blog_block_image_label")}{" "}
                        <span className="text-red-500">*</span>
                      </>
                    }
                  />
                  <div className="pt-4">
                    ...
                    >
                      {editingBlockIndex !== null
                        ? t("create_blog_block_update_btn")
                        : t("create_blog_block_add_btn")}
                    </button>
                  </div>
                </div>
              </Modal>
                ...
                >
                  {isSubmitting ? (
                    <>
                      ...
                      {t("create_blog_saving")}
                    </>
                  ) : (
                    <>
                      <CheckCircle size={20} />
                      {t("create_blog_submit_btn")}
                    </>
                  )}
                </button>
              </div>
