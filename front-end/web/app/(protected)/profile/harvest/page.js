...
export default function DiaryPage() {
  ...
  const selectableGardens = useMemo(
    () =>
      gardens.filter(
        (g) =>
          !["Stopped", "Completed", "Ngưng hoạt động"].includes(g.diary.status),
      ),
    [gardens],
  );

  const effectiveSelectedId = useMemo(() => {
    if (!selectableGardens.length) return null;
    if (
      selectedId &&
      selectableGardens.some((g) => g.diary.id === selectedId)
    ) {
      return selectedId;
    }
    return selectableGardens[0].diary.id;
  }, [selectableGardens, selectedId]);
  ...
  const selectedGarden =
    selectableGardens.find((g) => g.diary.id === effectiveSelectedId) ??
    selectableGardens[0] ??
    null;
  ...
  if (isSeasonDiariesLoading && !selectableGardens.length) {
    return (
      <div className="min-h-[50vh] flex items-center justify-center h-screen">
