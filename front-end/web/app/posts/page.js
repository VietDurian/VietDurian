      ...
      {expanded && (
        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {CATEGORY_GUIDE.map((cat) => {
            ...
            return (
              <button key={cat.key} onClick={() => onCategoryChange(isActive ? "Tất cả" : cat.key)} className={`group relative flex flex-col gap-3 p-4 rounded-2xl border-2 transition-all text-left shadow-sm hover:shadow-md ${isActive ? `${cat.bgLight} ${cat.borderColor} ring-2 ${cat.ringColor}` : "bg-white border-gray-200"}`}>
                <div className="flex items-start justify-between">
                  <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${cat.gradient} flex items-center justify-center shadow-md`}><Icon size={22} className="text-white" /></div>
          ...
          {showSortDropdown && (
            <div className="absolute top-full right-0 mt-2 w-full bg-white border-2 border-gray-200 rounded-lg shadow-xl z-50 overflow-hidden">
              {SORT_OPTIONS.map((option, index) => (
                <button key={option.value} onClick={() => { onSortChange(option.value); setShowSortDropdown(false); }} className={`w-full px-4 py-2.5 text-left text-sm transition-all duration-150 ${selectedSort === option.value ? "bg-emerald-50 text-emerald-700 font-semibold" : "text-gray-700 hover:bg-gray-50"} ${index !== SORT_OPTIONS.length - 1 ? "border-b border-gray-100" : ""}`}>{option.label}</button>
              ))}
            </div>
          )}
        </div>
...
export default function PostsContent() {
  ...
  useEffect(() => {
    ...
    const loadPostsWithFavorites = async () => {
      ...
      try {
        ...
        const favoritePostIds = new Set((favoritesResponse.data || []).map((fav) => fav.post_id?._id || fav.post_id).filter(Boolean));
        ...
        const postsWithComments = await Promise.all(normalizedPosts.map(async (post) => {
          try {
  ...
  const handleLikeUpdate = (postId, isLiked) => {
    setPosts((prev) => prev.map((post) => post.id === postId ? { ...post, isLiked, likes: isLiked ? post.likes + 1 : post.likes - 1 } : post));
  };

  const handleClearFilters = () => { setSelectedCategory("Tất cả"); setSelectedSort("newest"); setSearchQuery(""); };
