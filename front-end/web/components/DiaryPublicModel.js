          ...
          {rows.map((row, i) => (
            ...
            >
              ...
              {columns.map((col) => (
                <td
                  key={col.key}
                  className="px-3 py-2.5 text-gray-700 border-r border-gray-100"
                >
                  {col.render
...
function BuyingSeedSection({ diaryId }) {
  ...
  return (
    ...
    >
      {loading ? (
        <LoadingState msg={t("diary_loading")} />
      ) : error ? (
        <ErrorState msg={error} />
      ) : (
        <DataTable columns={cols} rows={rows} emptyMsg={t("diary_empty")} />
      )}
    </Section>
  );
}
...
function LaborCostSection({ diaryId }) {
  ...
  const cols = [
    {
      key: "labor_hire_date",
      label: t("diary_col_labor_hire_date"),
      minWidth: 100,
      render: fmtDate,
    },
    {
