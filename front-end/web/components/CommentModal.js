...
const getReactionIcon = (type, size = 18, filled = false) => {
    ...
    switch (type) {
        case "like":
            return <ThumbsUp {...iconProps} className={filled ? "fill-blue-500 text-blue-500" : "text-blue-500"} />;
        case "love":
            return <Heart {...iconProps} className={filled ? "fill-red-500 text-red-500" : "text-red-500"} />;
        case "haha":
...
const CommentItem = ({
    ...
    const isOwnComment = authorId === user?._id;
                ...
                <div className="flex items-center gap-5 mt-1.5 ml-3 text-xs relative">
                    ...
                    {showReactionOptions[comment._id] && (
                        <div className="absolute bottom-6 left-7 bg-white rounded-full shadow-lg px-2 py-1.5 flex gap-2 z-10 border border-gray-100">
                            ...
                            {userReaction && (
                                <button
                                    onClick={() => onReactionPress(comment._id, userReaction.type)}
...
export default function CommentModal({ isOpen, onClose, postId, post, onCommentCountChange }) {
    ...
    const handleReactionPress = async (commentId, reactionType) => {
        const commentReactions = reactions[commentId];
        const userReaction = commentReactions?.reactions?.find((r) => r.user_id?._id === user?._id || r.user_id === user?._id);
        ...
        try {
            if (userReaction) {
                if (userReaction.type === reactionType) {
                    ...
                    // Update reaction
                    await reactionCommentAPI.updateReaction(userReaction._id, reactionType);
                }
            } else {
                // Add new reaction - GỬI userId thay vì user_id
                await reactionCommentAPI.addReaction({
                    comment_id: commentId,
                    userId: user._id,  // ĐỔI TỪ user_id THÀNH userId
                    type: reactionType,
                });
            }
                    ...
                    <div className="border-t border-gray-200 p-4">
                            ...
                            <div className="flex items-center justify-between bg-emerald-50 rounded-lg px-3 py-2 mb-3">
                                <p className="text-sm text-gray-700">
                                    ...
                                    <span className="font-semibold">
                                        {replyingTo.author_id?.full_name || replyingTo.author_id?.username}
                                    </span>
                                </p>
                                <button onClick={handleCancelReply} className="text-gray-500 hover:text-gray-700">
                                    <X size={18} />
                                </button>
                            </div>
                        )}
                        ...
                        {editingComment && (
                            <div className="flex items-center justify-between bg-orange-50 rounded-lg px-3 py-2 mb-3">
                                <p className="text-sm text-gray-700 font-medium">Chỉnh sửa bình luận</p>
                                ...
                                <textarea
                                    ...
                                    placeholder={
                                        editingComment
                                            ? "Edit your comment..."
                                            : replyingTo
                                                ? `Reply to ${replyingTo.author_id?.username || "user"}...`
                                                : "Viết bình luận..."
                                    }
