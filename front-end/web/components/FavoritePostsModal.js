...
import {
    Heart, MessageCircle, ImageIcon, AlertCircle, Loader2,
    MoreHorizontal, CheckCircle, Clock, XCircle, X,
    Wrench, BookOpen, Package, LayoutGrid, HandCoins,
    Phone, Mail,
} from "lucide-react";
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/50">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                ...
                <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 px-4 py-2 rounded-lg border-2 border-gray-200 text-gray-700 font-medium hover:bg-gray-50 transition text-sm">{t('fav_posts_delete_cancel')}</button>
                    <button onClick={onConfirm} className="flex-1 px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white font-medium transition text-sm">{t('fav_posts_delete_btn')}</button>
                </div>
            </div>
        </div>
    );
...
const StatusBadge = ({ status }) => {
    ...
    return (
        <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border ${config.bgColor} ${config.borderColor} ${config.textColor} text-xs font-medium`}>
            <Icon size={14} className={config.iconColor} /><span>{config.text}</span>
        </div>
    );
};
...
const EditPostModal = ({ isOpen, onClose, post, user, onPostUpdated }) => {
    ...
    const validateContact = (type, value) => {
        ...
        if (type === "phone") {
            if (!PHONE_REGEX.test(value)) return t('fav_posts_phone_invalid');
        } else {
            if (!EMAIL_REGEX.test(value)) return t('fav_posts_email_invalid');
        }
        return "";
    };
    ...
    const handleImageChange = (event) => {
        ...
        reader.onload = () => { const result = reader.result?.toString() || ""; setImageData(result); setImagePreview(result); };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
            <div className="bg-white text-black w-full max-w-2xl rounded-xl shadow-2xl overflow-hidden">
                <div className="relative flex items-center justify-center p-4 border-b border-gray-200">
                    ...
                    <button onClick={onClose} className="absolute right-4 p-2 bg-gray-200 hover:bg-gray-300 rounded-full transition" aria-label="Đóng"><X size={20} className="text-gray-600" /></button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 p-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder={t('fav_posts_edit_desc_placeholder')} className="w-full bg-white text-gray-900 text-base resize-none outline-none min-h-[140px] placeholder:text-gray-500 border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-emerald-600" maxLength={1000} />
                    ...
                    <button type="submit" disabled={!canSubmit || isSubmitting} className="w-full bg-emerald-700 text-white font-bold py-3 rounded-lg hover:bg-emerald-800 transition disabled:opacity-60 disabled:cursor-not-allowed">
                        {isSubmitting
                            ? <span className="flex items-center justify-center gap-2"><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div>{t('fav_posts_edit_submitting')}</span>
...
const FavoritePostCard = ({ post, onToggleFavorite, onEdit, onDeleteConfirm, onContact }) => {
    ...
    const handleToggleFavorite = async () => {
        ...
        try { await onToggleFavorite(post.post_id?._id || post.post_id); setIsLiked(false); }
    ...
    const formatTimestamp = (dateString) => {
        ...
        try { return new Date(dateString).toLocaleString("vi-VN"); } catch { return t('fav_posts_unknown_time'); }
    };
    ...
    const getAuthorInfo = () => {
        ...
        if (postData.author_id && typeof postData.author_id === 'object') {
            ...
            return { name: postData.author_id.full_name || postData.author_id.name || "Người dùng", avatar: postData.author_id.avatar || "/images/avatar.jpg", handle: email || postData.author_id.username || "" };
        }
        ...
        if (authorId === user?._id || authorId === user?.id) {
            ...
            return { name: user.full_name || user.name || "Bạn", avatar: user.avatar || "/images/avatar.jpg", handle: email || user.username || "" };
        }
            ...
            <article className="bg-white border border-gray-200 rounded-2xl p-5 mb-5 shadow-sm hover:shadow-md transition-shadow w-full">
                <div className="flex justify-between items-start mb-3">
                    <div className="flex gap-3 flex-1 min-w-0">
                        <div className="w-11 h-11 rounded-full overflow-hidden shrink-0 ring-2 ring-gray-100 cursor-pointer" onClick={() => { const authorId = post.post_id?.author_id?._id || post.post_id?.author_id; if (authorId) router.push(`/profile/${authorId}`); }}>
                                ...
                                {cfg && (() => {
                                    ...
                                    return (
                                        <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r ${cfg.bg} text-white shadow-sm shrink-0`}>
                                            <Icon size={11} />{post.post_id.category}
                                        </span>
                                    );
                                })()}
                ...
                {post.post_id?.image && (
                    <div className="rounded-xl overflow-hidden mb-4 border border-gray-200">
                        <img src={post.post_id.image} alt="Post content" className="w-full h-auto object-cover" />
                    </div>
                )}
                ...
                <div className="pt-3 border-t border-gray-200 flex items-center justify-between px-1">
                    ...
                    <button onClick={() => onContact?.(post)} className="px-4 py-2 bg-emerald-600 text-white rounded-full font-medium hover:bg-emerald-700 transition-colors text-sm">{t('fav_posts_contact_btn')}</button>
                </div>
            </article>
...
export default function FavoritePostsModal() {
    ...
    const loadFavorites = async () => {
        try {
            ...
            const favWithComments = await Promise.all(validFavorites.map(async (fav) => {
                try {
    ...
    const handleDelete = async () => {
        ...
        } catch (err) { toast.error(err?.message || t('fav_posts_delete_fail')); }
    };
    const handleContact = (post) => {
        ...
        addContact(chatUser); setSelectedUser(chatUser); router.push(`/chat/${authorId}`);
    };

    if (loading) return <div className="flex flex-col items-center justify-center py-12"><Loader2 className="w-10 h-10 text-emerald-600 animate-spin mb-3" /><p className="text-gray-500 font-medium">{t('fav_posts_loading')}</p></div>;
    if (error) return <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center"><AlertCircle className="mx-auto text-red-500 mb-3" size={32} /><p className="text-red-600 font-medium">{error}</p><button onClick={loadFavorites} className="mt-4 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition">{t('fav_posts_retry')}</button></div>;
