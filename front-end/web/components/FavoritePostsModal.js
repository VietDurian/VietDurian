...
const StatusBadge = ({ status }) => {
  ...
  const statusConfig = {
    pending: {
      icon: Clock,
      text: t("fav_posts_status_pending"),
      bgColor: "bg-amber-50",
    ...
    active: {
      icon: CheckCircle,
      text: t("fav_posts_status_active"),
    ...
    rejected: {
      icon: XCircle,
      text: t("fav_posts_status_rejected"),
      bgColor: "bg-red-50",
    ...
    inactive: {
      icon: AlertCircle,
      text: t("fav_posts_status_inactive"),
      bgColor: "bg-gray-50",
      textColor: "text-red-700",
      iconColor: "text-red-500",
      borderColor: "border-red-200",
...
const EditPostModal = ({ isOpen, onClose, post, user, onPostUpdated }) => {
  ...
  const [contactType, setContactType] = useState(
    detectContactType(post?.contact),
  );
  ...
  const handleImageChange = (event) => {
    ...
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) {
      setError(t("fav_posts_edit_image_too_large"));
      return;
    }
    const reader = new FileReader();
    reader.onload = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!canSubmit) {
      setError(t("fav_posts_edit_required"));
      return;
    }
    setIsSubmitting(true);
    try {
  ...
  : t("post_service_provide_hint")}
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-gray-700">
              {t("fav_posts_edit_contact")}
            </label>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => handleContactTypeChange("phone")}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${contactType === "phone" ? "border-emerald-600 bg-emerald-50 text-emerald-700" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}
              >
                <Phone size={15} />
                {t("fav_posts_edit_phone")}
              </button>
              <button
                type="button"
                onClick={() => handleContactTypeChange("email")}
                className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 text-sm font-medium transition-all ${contactType === "email" ? "border-emerald-600 bg-emerald-50 text-emerald-700" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}
              >
                <Mail size={15} />
                {t("fav_posts_edit_email")}
              </button>
            </div>
            <input
              ...
              placeholder={
                contactType === "phone"
                  ? t("fav_posts_edit_phone_placeholder")
          ...
          <div className="space-y-2">
            ...
            {!imagePreview ? (
              ...
              >
                ...
                <p className="text-sm font-medium text-gray-600">
                  {t("fav_posts_edit_image_click")}
                </p>
                <p className="text-xs text-gray-400 mt-1">
                  {t("fav_posts_edit_image_hint")}
                </p>
              </div>
            ) : (
              <div className="relative rounded-lg overflow-hidden border border-gray-200">
...
const FavoritePostCard = ({
  ...
  const formatTimestamp = (dateString) => {
    ...
    try {
      ...
      return t("fav_posts_unknown_time");
    }
  };
  ...
  const getAuthorInfo = () => {
    ...
    const authorId =
      typeof postData.author_id === "string"
        ? postData.author_id
  ...
  const isOwnPost = () => {
    const postAuthorId =
      post.post_id?.author_id?._id || post.post_id?.author_id;
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-500 text-sm truncate">
                ...
                {post.post_id?.created_at &&
                  formatTimestamp(post.post_id.created_at)}
              </p>
            </div>
          </div>
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowMenu(false);
                        onDeleteConfirm?.(post.post_id?._id);
                      }}
...
export default function FavoritePostsModal() {
    ...
    try {
      ...
      const favWithComments = await Promise.all(
        validFavorites.map(async (fav) => {
          try {
            const res = await commentAPI.getCommentsByPost(
              fav.post_id?._id,
              "all",
            );
            ...
            return {
              ...
              post_id: {
                ...fav.post_id,
                comments_count: countComments(res.data || []),
              },
            };
          } catch {
            return fav;
  ...
  const handleToggleFavorite = async (postId) => {
    try {
      ...
      toast.error(
        err?.response?.data?.message || err?.message || t("fav_posts_fav_fail"),
      );
    }
  };
    ...
    setFavorites((prev) =>
      prev.map((fav) =>
        fav.post_id._id === updatedPost._id
          ? { ...fav, post_id: { ...fav.post_id, ...updatedPost } }
          : fav,
      ),
    );
  };
    ...
    try {
      ...
      setFavorites((prev) =>
        prev.filter((fav) => {
          ...
          return favPostId !== deletingPostId;
        }),
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmModal
        ...
        message={t("fav_posts_delete_confirm")}
      />
    </div>
  );
}